...
特にレガシーで不要になった `ai-open-code-review.yml` を削除し、最新の Ollama モデル `qwen2.5-coder:0.5b` (コーディング特化) へのアップデートを行いました。また、週間トレンドの検索処理に無料枠で使える DuckDuckGo 検索（ddgs）を導入し、完全な無料運用を実現しています。
