...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/monopo`](https://github.com/genzouw/monopo) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
- **MUST NOT**: 上記の MUST NOT に抵触する既存ワークフロー（スケジュール実行を停止して `workflow_dispatch` のみ残しているものを含む）の再開・移行・再導入。新規の構成追加だけでなく、既存の非準拠ワークフローを復活・移行させる変更も同様に禁止します。撤去（削除）または完全無料の代替への置き換えのみ許可されます。
