...
## 生成AIツール（PR-Agent）導入に伴う事前作業

<!-- このリポジトリで初めてPRを作成する方、またはPR-Agentのレビューを有効化したい方向けの事前準備手順です。 -->

- [ ] リポジトリ（またはOrganization）のSettings > Secrets and variables > Actionsにて、`OPENAI_KEY`を設定しましたか？
  - ※この設定がないとPR-Agentによる自動レビューや要約生成は実行されません。
