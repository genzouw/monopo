...
本ワークフローは再開・移行・再導入しないでください。同種の自動化を新規に設計する場合は、Issue #411 で指摘された根本対策（AIにスクリプトを直接生成・実行させず、Unified Diff / Patch形式で出力させて検証後に適用する設計、または Deno 等のサンドボックスでアクセスを制限した実行環境を用いる設計）を反映してください。
