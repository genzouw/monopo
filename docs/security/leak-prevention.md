...
ただし、このルールは列挙された変数名（SECRET, PRIVATE, PASSWORD, CREDENTIAL, AUTH, TOKEN, API_KEY 等）と検知可能な値形式（10文字以上・限定文字集合）を対象とする補助的な検知であり、記号を含む値や未列挙の変数名までは検知できません。`VITE_` 変数には機密値を設定しないことを原則とし、Vercel や GitHub Secrets に設定した環境変数名についても、リポジトリ内スキャンの対象外であるため、マージ前に別途目視で確認してください。
