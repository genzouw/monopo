...
- **週次スケジュールスキャン**:
  - `gitleaks.yml`、`trufflehog.yml`、および `trivy.yml` には cron トリガー (毎週月曜日) が設定されており、Gitコミット履歴全体のスキャンおよび最新の脆弱性情報の取得を自動で行います。TruffleHog は定期監査においても有効なアクティブシークレットを継続的に監視します。
