...
- **Secretlint ワークフロー (`.github/workflows/ci.yml`)**:
  - `main` ブランチへの push および `main` を対象にした PR で実行されるCIの中で `secretlint` のジョブを実行し、Node.js エコシステムに特化したシークレット漏洩スキャンを行います。これにより、ローカルのフックをすり抜けた場合でも確実に検知・ブロックします。
  - **注意**: `gitleaks` や `trivy` とは異なり、`secretlint` は `ci.yml` の他のジョブ（lint/format/typecheck/build/test 等）と同一の `on` トリガーを共有しているため、対象ブランチは `main` に限定されています（フィーチャーブランチでの漏洩検知は `gitleaks` / `trivy` が担います）。
