...
- **`actionlint` および `zizmor` フック (pre-commit)**: `.github/workflows/` 配下の YAML ファイルに対して、コミット前に `actionlint` と `zizmor` を実行します。
  ...
  - **バージョン固定の整合性検証 (`actionlint-pin-consistency`)**: `actionlint-docker` の `entry` は digest 固定（CWE-494: 整合性チェックなしのコード取得対策）していますが、Renovate の pre-commit 更新は `rev` 行しか書き換えません。`rev` だけが上がって `entry` が古いまま残ると、フック定義と実際に実行されるイメージがずれます。`scripts/check-actionlint-pin.sh` を実行するローカルフックで両者のバージョン一致を検証し、不一致のまま `.pre-commit-config.yaml` を変更できないようにしています。
