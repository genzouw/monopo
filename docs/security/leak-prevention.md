...
### GitHub Actions 権限の最小化

CI設定を通じた意図せぬリポジトリへの書き込みや情報漏洩リスク（例えば、サードパーティのアクションの脆弱性やインジェクション攻撃による権限昇格）を防止するため、全ワークフローファイルにおいて **権限の最小化（Least Privilege）** を徹底しています。

- **トップレベルでの権限制限**: 全ての `.github/workflows/*.yml` において、トップレベルの `permissions:` は原則として `contents: read` （読み取り専用）等の必要最小限に制限します。
- **ジョブレベルでの権限付与**: 特定のジョブ（例：Issueへのコメント作成やPRの作成）で `write` 権限など追加のアクセスが必要な場合のみ、該当するジョブ内の `permissions:` ブロックで明示的に許可を付与しています。
- **定期監査**: `.github/workflows/permissions-audit.yml` などにより、ワークフローの権限設定が適切に行われているか（トップレベルの permissions が存在するか）を継続的に監査しています。
