...
- **VS Code / Cursor 用安全側プリセット (`.vscode/settings.json`)**: リポジトリに事前に設定されたプリセットにより、ローカルエディタのエクスプローラーからシークレットファイル (`.env`, `*.pem`等) やAIエージェントの作業跡 (`.cursor/`, `.claude/`等) を除外 (`files.exclude`) し、誤操作によるコミットを未然に防ぎます。
