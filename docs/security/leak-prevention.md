...
- **`pre-commit` framework**: `.husky/pre-commit` を通して `.pre-commit-config.yaml` による標準的なフック（秘密鍵の検知、YAML構文チェックなど）に加え、`trufflehog`、`detect-secrets` など多層的なシークレット検知が行われ、コミット前の安全性をさらに高めています。`gitleaks` と同様にインストールが**必須**となっており、未インストールの場合はコミットがブロックされます（例: `brew install pre-commit` または `pip install pre-commit`）。
  ...
  - **自動依存解決**: `pre-commit` framework が実行する gitleaks フックには、システム依存の `gitleaks-system` ではなく、pre-commit が自動で依存関係を解決して実行する `gitleaks` を使用することで、CI 環境や新規開発者の環境での実行エラーを防ぎ、安定性を向上させています（なお、上記「`gitleaks` フック」の項に記載の `.husky/pre-commit` 経由の gitleaks 実行には、引き続きローカルへの gitleaks インストールが必要です）。
- **`actionlint` および `zizmor` フック (pre-commit)**: `.github/workflows/` 配下の YAML ファイルに対して、コミット前に `actionlint` と `zizmor` を実行します。
  ...
  - **バージョン固定の整合性検証 (`actionlint-pin-consistency`)**: `actionlint-docker` の `entry` は digest 固定（CWE-494: 整合性チェックなしのコード取得対策）していますが、Dependabot の pre-commit 更新は `rev` 行しか書き換えません。`rev` だけが上がって `entry` が古いまま残ると、フック定義と実際に実行されるイメージがずれます。`scripts/check-actionlint-pin.sh` を実行するローカルフックで両者のバージョン一致を検証し、不一致のまま `.pre-commit-config.yaml` を変更できないようにしています。
...
**pip 対象ファイルに関する既知の制約**: `requirements-ddgs.txt` はファイル先頭のコメントの通り `pip-compile --allow-unsafe --generate-hashes --no-index --output-file=requirements-ddgs.txt requirements.in` で生成されたハッシュ固定ファイルですが、対応する `requirements.in` はリポジトリに含まれていません。dependabot-core の pip アップデータは pip-compile 形式のファイルを検知すると対になる `.in` を参照して再コンパイルする設計のため、`.in` が存在しないこの構成では定期実行（毎週月曜日）で更新 PR が生成されない可能性があります。次回実行後に更新 PR が確認できない場合は、まず `requirements.in` をリポジトリに追加してください。なお dependabot-core の pip アップデータは再コンパイル時に常に自前の `--index-url` を注入する実装のため、`--no-index` の除去は必須の対応ではなく、`requirements.in` 追加後も更新 PR が生成されない場合にのみ検討する候補として位置づけてください。
...
### 追加のカスタム漏洩検知・抑止対策 (Gitleaks 強化 - 最新 AI プロバイダ・エディタ・Cloudflare 対応)
...
- `${...}` のような環境変数参照、`<REDACTED>`、および `your` / `my` / `change-me` / `placeholder` / `sample` / `example` / `todo` / `dummy` / `test` / `foo` / `bar` / `here` / `please` / `api` / `key` / `token` / `secret` / `value` / `password` **といったプレースホルダー語だけを区切り文字（`-` / `_` / `.`）で連結した値**（例: `your_token_here` / `my-api-key` / `CHANGE_ME_PLEASE` / `dummy-password`）や `x` を8文字以上連続する値（`.env.example` の定番プレースホルダー。allowlist により除外）。プレースホルダー語で始まっても、その先にプレースホルダー語以外が続く値（例: `mySuperSecretValue123` / `my-real-production-token-a1b2c3d4` <!-- pragma: allowlist secret -->）は**実在シークレットとみなして検知対象のまま**です。この allowlist は `monopo-ai-token-assignment-extended` / `monopo-cloudflare-token-assignment` / `monopo-frontend-exposed-secret` の3ルールで**同一内容に同期**しており、同じ値がルールによって検知されたりされなかったりしないようにしています（同期は `scripts/test-gitleaks-rules.sh` が regex 行の出現回数を assert しているため、語を1つ増減して片方だけ更新すると CI で失敗します）
- `VITE_FIREBASE_AUTH_DOMAIN` / `NEXT_PUBLIC_FIREBASE_API_KEY` のように、Firebase Web の `authDomain`・`apiKey` など**仕様上ブラウザに公開される値**を保持する変数名（`VITE_` `NEXT_PUBLIC_` `EXPO_PUBLIC_` `NUXT_PUBLIC_` `GATSBY_` `REACT_APP_` `VUE_APP_` `NG_APP_` `PUBLIC_` のいずれのプレフィックスでも allowlist により除外されます。ただし除外後も、値の形状次第では gitleaks 既定の `generic-api-key` ルールでは引き続き検知され得ます）
- `NEXT_PUBLIC_AUTH0_DOMAIN` / `NEXT_PUBLIC_AUTH0_CLIENT_ID` / `NEXT_PUBLIC_AUTH0_AUDIENCE` のような `AUTH0_` 系の変数名。`AUTH` キーワードは `AUTH(?:_|\b)` と終端を限定しており、直後が数字（`AUTH0`）の場合は一致しません。Auth0 SPA の domain・clientId・audience はブラウザに公開される設定値であり、変数名を allowlist へ足し続ける運用を避けるための設計です（機密値である `AUTH0_CLIENT_SECRET` は `SECRET` キーワードで引き続き検知されます）
- `PUBLIC_AUTHOR_ID` / `NEXT_PUBLIC_AUTHOR_HANDLE` のように `AUTHOR` を含むだけの変数名。上記と同じ `AUTH(?:_|\b)` の境界により、`AUTH` の部分一致では検知しません（`GATSBY_BASIC_AUTH` や `VITE_AUTH_SERVER_DOMAIN` は引き続き検知対象です）
