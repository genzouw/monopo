...
- `VITE_FIREBASE_AUTH_DOMAIN` や `NEXT_PUBLIC_AUTH0_DOMAIN` / `NEXT_PUBLIC_AUTH0_CLIENT_ID` / `NEXT_PUBLIC_AUTH0_AUDIENCE` のように、Firebase Web の `authDomain` や Auth0 SPA の domain・clientId・audience など**仕様上ブラウザに公開される値**を保持する変数名（`VITE_` `NEXT_PUBLIC_` `EXPO_PUBLIC_` `NUXT_PUBLIC_` `GATSBY_` `REACT_APP_` `VUE_APP_` `NG_APP_` `PUBLIC_` のいずれのプレフィックスでも allowlist により除外されます。ただし除外後も、値の形状次第では gitleaks 既定の `generic-api-key` ルールでは引き続き検知され得ます）
