...
- `${...}` のような環境変数参照、`<REDACTED>`、および `your` / `my` / `change-me` / `placeholder` / `sample` / `example` / `todo` / `dummy` / `test` / `foo` / `bar` / `here` / `please` / `api` / `key` / `token` / `secret` / `value` / `password` **といったプレースホルダー語だけを区切り文字（`-` / `_` / `.`）で連結した値**（例: `your_token_here` / `my-api-key` / `CHANGE_ME_PLEASE` / `dummy-password`）や `x` を8文字以上連続する値（`.env.example` の定番プレースホルダー。allowlist により除外）。プレースホルダー語で始まっても、その先にプレースホルダー語以外が続く値（例: `mySuperSecretValue123` / `my-real-production-token-a1b2c3d4` <!-- pragma: allowlist secret -->）は**実在シークレットとみなして検知対象のまま**です。この allowlist は `monopo-ai-token-assignment-extended` / `monopo-cloudflare-token-assignment` / `monopo-frontend-exposed-secret` の3ルールで**同一内容に同期**しており、同じ値がルールによって検知されたりされなかったりしないようにしています
