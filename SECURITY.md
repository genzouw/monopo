...
- **CI 検知**: GitHub Actions にて `gitleaks`、`trivy`、`trufflehog` を用い、PR差分およびリポジトリ全体の履歴・依存関係に含まれるシークレットを自動スキャンしています。また、`actionlint` を `shellcheck` や `pyflakes`（静的解析ツール）と連携させ、CI スクリプト経由のリスク（シェルインジェクションや Python スクリプトの不適切な記述等）を検知・抑止しています。さらに、`.env` や各種キーファイル、AI エージェントの作業ディレクトリ（`.cursor/` 等）といった特定ファイルの混入を CI および pre-commit hook で明示的に検知・ブロックするファイル名ベースの多層防御を行っています。
...
### AI ツール統合のセキュリティと SHA ピン留め

GitHub Actionsを用いたAIコードレビューツール（PR-Agent / CodeRabbitなど）の統合においても、すべてのサードパーティ製 Action は mutable なタグ（`@v1`、`@main` など）ではなく、正確な 40文字のコミット SHA にピン留めすることが義務付けられています。これにより、ツールに悪意のあるコードが紛れ込むことや、プロンプトインジェクション、データ漏洩などの AI 固有のセキュリティリスクを低減しています。
