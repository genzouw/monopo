...
- **ローカル検知**: `.husky/pre-commit` および `.husky/commit-msg` フックにより、`gitleaks` を用いたコミット前のソースコードやコミットメッセージ自体のシークレット混入検知を必須化しています。さらに、Node.js エコシステムに特化した `secretlint` を `lint-staged` に統合し、コミット対象のファイルに対する漏洩検知を二重化しています。
