...
- **データセキュリティ/PII検知**: `Bearer` を CI に導入し、ソースコード上の個人情報 (PII) やビジネスロジックにおけるデータセキュリティリスクを定期的に静的解析 (SAST) しています。
...
- **定期監査**: 週次ベースでリポジトリ全体のフルスキャンを自動実行し、設定ミスによる新たな漏洩リスクを継続的に監視しています。また、`osv-scanner` を用いた依存パッケージのOSS脆弱性走査や、`license-checker-rseidelsohn` を用いたライセンスコンプライアンスの定期走査も行い、サプライチェーン・コンプライアンスリスクの低減を図っています。 さらに、`CodeAnt AI` による SAST/SCA/シークレットスキャンも CI 上の `push` や `pull_request` 時に加え、週次の `schedule` トリガーで定期的に実行し、多層的な監査を行っています（`ACCESS_TOKEN_GITHUB` が前提条件であり、未設定の場合はワークフローが `::warning::` を出力したうえでスキャンをスキップします。設定手順は `docs/security/ai-ci-tools.md` を参照してください）。さらに、`ossf/scorecard-action` を用いてリポジトリのセキュリティ設定・プラクティスの健全性を継続的に評価・スコアリングしています。
